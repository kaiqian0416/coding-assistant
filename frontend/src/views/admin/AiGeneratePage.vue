<template>
  <div class="admin-page">
    ...
    <div class="card">
      ...
      <form @submit.prevent="handleGenerate">
        <div class="form-group">
          <label>知识点（可手动输入自定义知识点）</label>
          <input v-model="form.knowledgePoint" class="form-input" list="kp-list" placeholder="选择已有或输入新知识点" required />
          <datalist id="kp-list">
        ...
        <div class="form-group">
          ...
          <p style="font-size:12px;color:var(--text-muted);margin-top:4px;">简要描述你想要题目的核心主题</p>
        </div>
    ...
    <div v-if="generatedQuestion" class="card">
      ...
      <div class="form-group"><label>标题</label><input :value="generatedQuestion.title" class="form-input" disabled /></div>
      <div class="form-group"><label>题目描述</label><textarea :value="generatedQuestion.description" class="form-textarea" rows="4" disabled></textarea></div>
      <div class="form-group"><label>样例输入</label><textarea :value="generatedQuestion.sampleInput" class="form-textarea" rows="2" disabled></textarea></div>
      <div class="form-group"><label>样例输出</label><textarea :value="generatedQuestion.sampleOutput" class="form-textarea" rows="2" disabled></textarea></div>
      <div class="form-group"><label>参考代码</label><textarea :value="generatedQuestion.referenceCode" class="form-textarea" rows="6" disabled></textarea></div>
...
const generating = ref(false); const error = ref(''); const success = ref('')
const generatedQuestion = ref<any>(null)
const knowledgePoints = ['基础语法', '条件判断', '循环结构', '字符串处理', '数组操作', '递归与递推', '排序算法', '查找算法', '动态规划', '图算法']

async function handleGenerate() {
  generating.value = true; error.value = ''; success.value = ''; generatedQuestion.value = null
  try {
    const res = await adminApi.generateQuestion(form)
    if (res.data.code === 200) { generatedQuestion.value = res.data.data; success.value = '题目生成成功！请预览并审核。' }
...
async function handleApprove(id: number) {
  try { await adminApi.approveQuestion(id); success.value = '已审核通过！'; generatedQuestion.value = null } catch (e) {}
}
async function handleReject(id: number) {
  try { await adminApi.rejectQuestion(id); success.value = '已驳回'; generatedQuestion.value = null } catch (e) {}
}
</script>
